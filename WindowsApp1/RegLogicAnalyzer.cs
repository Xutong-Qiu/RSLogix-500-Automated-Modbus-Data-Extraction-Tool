    ...
    public static class RegLogicAnalyzer
    {
        ...
        private static bool RegPattern01(Node root, List<Tuple<string, string>> results)
        {
            ...
            if (cur is not null && cur.Ins == "CPW")
            {
                ...
                string des = AddressSolver.Tune(cur.Args[1]);
                int offset = (int)Math.Round(Conversions.ToSingle(cur.Args[2]) - 1f);
                results.Add(new Tuple<string, string>(src, des));
                results.Add(new Tuple<string, string>(src, AddressSolver.AddrAdder(des, offset)));
                return true;
            }
            return false;
        }
        ...
        private static bool RegPattern0(Node root, List<Tuple<string, string>> results)
        {
            ...
            {
                results.Add(new Tuple<string, string>(cur.Args[0], AddressSolver.Tune(cur.Args[1])));
                return true;
            }
            return false;
        }
        ...
        private static bool RegPattern3(Node root, List<Tuple<string, string>> results)
        {
            ...
            if (cur is not null && cur.Ins == "EQU")
            {
                ...
                {
                    var branch1 = cur.Children[0];
                    var branch2 = cur.Children[1];
        ...
        private static bool RegPattern5(Node root, List<Tuple<string, string>> results)
        {
            ...
            if (cur is not null && cur.Ins == "CPW")
            {
                ...
                results.Add(new Tuple<string, string>("CPW", AddressSolver.AddrAdder(des, offset))); // the second one treated as an exception
                return true;
            }
            return false;
        }
