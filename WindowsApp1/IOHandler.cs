    ...
    /// <param name="db">The database</param>
        public static void WriteToProject(object proj, PLC_DB db)
        {
            ...
            foreach (var addr in modifiedList)
            {
                ...
                if (Conversions.ToBoolean(!record.SetSymbol(db.GetTagName(addr)) && (object)string.IsNullOrEmpty(db.GetTagName(addr))))
                {
                    MessageBox.Show("Unable to set Name: " + db.GetTagName(addr) + "Address: " + addr);
                }
                if (Conversions.ToBoolean(!record.SetDescription(db.GetDescription(addr)) && (object)string.IsNullOrEmpty(db.GetDescription(addr))))
                {
