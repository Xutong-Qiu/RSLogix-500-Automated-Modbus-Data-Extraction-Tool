...
/// </summary>
    ...
    {
        public static void FindCoilMapping(WindowsApp1.PLC_DB DB, Node root, List<Tuple<string, string>> results)
        {
            var cur = root;
            int bit = 0;
        ...
        private static bool CoilPattern1(Node root, List<Tuple<string, string>> results, int bit)
        {
            ...
            if (cur is not null && cur.Ins == "XIC")
            {
                ...
                {
                    results.Add(new Tuple<string, string>(src, WindowsApp1.AddressSolver.Tune(cur.Args(0) + "/" + bit)));
                    return true;
                }
            }
            return false;
        ...
        // EQU->OR
        private static bool CoilPattern2(WindowsApp1.PLC_DB DB, Node root, List<Tuple<string, string>> results, int bit)
        {
            var cur = root;
            if (cur is not null && cur.Ins == "EQU")
            {
        ...
        private static bool CoilPatternSWP(Node root, List<Tuple<string, string>> results)
        {
            ...
            if (cur is not null && cur.Ins == "BST")
            {
                if (cur.Children.Count == 4)
                {
                    ...
                    {
                        string src = WindowsApp1.AddressSolver.Tune(cur.Children(0).Args(0));
                        string des = WindowsApp1.AddressSolver.Tune(cur.Children(2).Args(1));
                        if ((src ?? "") != (des ?? ""))
                        {
        ...
        private static bool HandlesExceptionMapping(Node root, List<Tuple<string, string>> results, int bit)
        {
            ...
            if (cur is not null && cur.Ins == "OR")
            {
                results.Add(new Tuple<string, string>("Logic involved", WindowsApp1.AddressSolver.Tune(cur.Args(0)) + "/" + bit));
                return true;
            }
            else if (cur is null)
            {
